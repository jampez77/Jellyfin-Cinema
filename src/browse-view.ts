...
export class BrowseView {
  ...
  private renderControls(): void {
    ...
    const tabs: [BrowseTab,string][] = this.options.kind === 'recordings'
      ...
      : [['albums','Albums'],['suggestions','Suggestions'],['albumArtists','Album artists'],['artists','Artists'],['songs','Songs'],['genres','Genres']];
    ...
    const nativeLinks = this.options.kind === 'recordings'
      ...
      : [['Playlists',`#/music?${new URLSearchParams({...(this.options.parentId?{topParentId:this.options.parentId}:{}),collectionType:'music',tab:'4'})}`]];
  ...
  private async fetch(append = false): Promise<void> {
    ...
    try {
      if (this.options.item && !['MusicAlbum','MusicArtist'].includes(this.options.item.Type || '')) {
        ...
        const found = new Map((append ? this.items : []).map(item => [item.Id,item]));
        let total = 0;
        do {
      ...
      if (focusRevision === this.focusRevision && (this.element.contains(document.activeElement) || document.activeElement === document.body)) {
        const target = append ? `item:${this.items[oldCount]?.Id || ''}` : requestedFocus;
        if (target) this.focus(target, true);
        else if (this.options.item) this.focus(playable(this.options.item) ? 'play' : '', true);
  ...
  private renderItems(): void {
    const name = this.options.item?.Type === 'MusicAlbum' ? 'Tracks' : this.options.item?.Type === 'MusicArtist' ? 'Albums'
      : this.state.search ? `Results for “${this.state.search}”` : this.state.genreName || (this.state.tab === 'songs' ? 'Songs' : this.options.kind === 'recordings' ? 'Your recordings' : this.state.tab === 'artists' || this.state.tab === 'albumArtists' ? 'Artists' : 'Albums');
    ...
    else this.results.append(this.empty(this.state.favorite?'No favourites found':'Nothing found',this.state.search||this.state.genreId||this.state.letter?'Try a different search or filter.':'Items in your Jellyfin library will appear here.'));
  ...
  private grid(items: Item[], prefix: string, row: boolean): HTMLElement {
    const grid = el('div',row?'tvl-browse-grid tvl-browse-row':'tvl-browse-grid'); grid.setAttribute('role','list');
    for (const item of items) {
      const entry=el('div','tvl-browse-entry');entry.setAttribute('role','listitem');
      const card=el('button','tvl-browse-card');card.type='button';card.dataset.browseItem=item.Id;card.dataset.itemType=item.Type||'';card.dataset.focusId=`${prefix}:${item.Id}`;
      const episode=item.Type==='Episode';const label=episode&&item.SeriesName?`${item.SeriesName}: ${item.Name}`:item.Name;
      card.setAttribute('aria-label',this.options.item?.Type==='MusicAlbum'?`Play ${label}`:label);
      ...
      card.append(copy);card.addEventListener('focus',()=>{if(!this.options.item)this.renderHero(item);});
      card.addEventListener('click',()=>{if(this.disposed)return;this.save();if(this.options.item?.Type==='MusicAlbum')void this.play(item);else this.options.navigate(item.Id);});
  ...
  private renderHero(item?: Item): void {
    ...
    if(item){
      ...
      if(playable(item)){const play=button(item.UserData?.PlaybackPositionTicks?'Resume':item.Type==='MusicAlbum'?'Play album':'Play','play','tvl-primary',()=>{void this.play(item);});play.dataset.focusId='play';actions.append(play);}
  ...
  private async play(item: Item): Promise<void> {
    ...
    try {
      await this.api.play(item,item.UserData?.Played?0:item.UserData?.PlaybackPositionTicks||0,current);
      ...
      if(['Audio','MusicAlbum'].includes(item.Type||''))finish('Playback requested. Open Now playing for controls.');
