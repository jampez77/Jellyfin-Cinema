...
test('guide banner follows the highlighted live or upcoming programme with its own artwork', async ({ page }) => {
  ...
  const composition = await banner.evaluate(node => {
    ...
    return {height: hero.height, imageHeight: art.height, imageTop: art.top, imageLeft: art.left, titleLeft: title.left, imageRight: art.right, viewportWidth: innerWidth, viewportHeight: innerHeight};
  });
  ...
  expect(composition.imageHeight).toBe(composition.viewportHeight);
