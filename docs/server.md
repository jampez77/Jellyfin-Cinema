...
The [v0.1.2 release](https://github.com/jampez77/Jellyfin-TV-Item-Layout/releases/tag/v0.1.2) adds theme-video visibility, a guide hero banner, series recommendations and collection links. Choose a plugin version that matches your Jellyfin server:
...
If TV Item Layout is already installed, update it from the catalogue, restart Jellyfin, and fully reopen the client. On Jellyfin 12 the new plugin version is **0.1.2.3**. In TV display mode, **Live TV** now opens our guide at `web/#/livetv?collectionType=livetv`, and **Channels & guide** on channel details links to the same page. Desktop and mobile layouts retain Jellyfin's normal pages.

Theme videos use Jellyfin’s existing background player and follow its theme-video preference. If there is no playable theme video, the static backdrop remains. Collections list actual membership and open Jellyfin’s normal collection pages.
