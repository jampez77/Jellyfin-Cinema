...
The current prerelease is **0.2.9**, available through the catalogue and manual downloads below. It extends the Cinema theme, collection rows, player features and **Who’s watching?** chooser to desktop with mouse and keyboard support. This release still needs installation and physical-TV testing.
...
Choose the build from the [v0.2.9 prerelease](https://github.com/jampez77/Jellyfin-Cinema/releases/tag/v0.2.9) that matches your Jellyfin server:
...
3. Download the matching archive from the [0.2.9 release assets](https://github.com/jampez77/Jellyfin-Cinema/releases/tag/v0.2.9), or use a matching local archive from `dist/releases/`. Stop Jellyfin. Extract the ZIP into a versioned folder inside your server's configured `plugins` directory, such as `TV Item Layout_0.2.9.3` for Jellyfin 12. Keep the DLL directly inside this folder and replace the previous version when upgrading manually. Common plugin locations are `/config/plugins` for the official container and `/var/lib/jellyfin/plugins` for a Linux package installation; use the location belonging to your installation.
