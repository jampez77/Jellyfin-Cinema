...
Choose the build from the [v0.2.6 prerelease](https://github.com/jampez77/Jellyfin-Cinema/releases/tag/v0.2.6) that matches your Jellyfin server:
...
3. Download the matching archive from the [0.2.6 release assets](https://github.com/jampez77/Jellyfin-Cinema/releases/tag/v0.2.6), or use a matching local archive from `dist/releases/`. Stop Jellyfin. Extract the ZIP into a versioned folder inside your server's configured `plugins` directory, such as `TV Item Layout_0.2.6.3` for Jellyfin 12. Keep the DLL directly inside this folder and replace the previous version when upgrading manually. Common plugin locations are `/config/plugins` for the official container and `/var/lib/jellyfin/plugins` for a Linux package installation; use the location belonging to your installation.
...
Jellyfin’s native music player and Now playing queue receive a larger album-art layout, clear track details and matching queue styling; their queue actions, playback state and permissions remain native. The current DVR library’s **Recordings** tile on Home uses Cinema’s recordings layout, scoped to that folder. An older recording library may remain a separate mixed-content folder after the DVR destination changes. In 0.2.6, those folders also receive Cinema headings, controls and cards while keeping their native contents and filters. No folders are merged or redirected. Recordings’ **Schedule**, **Series recordings**, details and DVR dialogs retain Jellyfin’s scheduling, editing and permission checks.

## Search, settings and profiles in 0.2.6

In TV mode, Search, Settings and the signed-out login page share the Cinema styling. Their native search controls, preference forms, Quick Connect and authentication behavior remain active. Desktop and mobile keep Jellyfin’s native pages and account menu.

Select the user avatar for **Switch profile**, **Settings**, or **Dashboard** when the signed-in account is an administrator. Settings also includes an administrator Dashboard shortcut; its permission is checked again when selected.

**Switch profile** shows Jellyfin’s public profiles. Select an account that Jellyfin already allows without a password to go directly to Home after the native session handover. Protected accounts and **Use login screen** open native sign-in, where password requirements still apply and hidden accounts can sign in manually. Cinema does not store additional credentials or change server password policies. Login styling follows the saved device TV display mode even while signed out.
